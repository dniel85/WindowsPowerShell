...
# --------------------
# STIG:     VMware Horizon 7.13 Client
# Version:  V1R1
# Class:    UNCLASSIFIED
# Updated:  4/25/2023
...
Function Get-V246875 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246875
        STIG ID    : HRZC-7X-000001
        Rule ID    : SV-246875r768585_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must not send anonymized usage data.
    #>
    ...
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\"  # Registry path identified in STIG
    $RegistryValueName = "AllowDataSharing"  # Value name identified in STIG
    $RegistryValue = @("false")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Allow data sharing"  # GPO setting name identified in STIG
...
Function Get-V246876 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246876
        STIG ID    : HRZC-7X-000002
        Rule ID    : SV-246876r768588_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must not connect to servers without fully verifying the server certificate.
    #>
    ...
    #---=== Begin Custom Code ===---#
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security\"  # Registry path identified in STIG
    $RegistryValueName = "CertCheckMode"  # Value name identified in STIG
    $RegistryValue = @("2")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Certificate verification mode"  # GPO setting name identified in STIG
    $SettingState = "Enabled with Full Security"  # GPO configured state identified in STIG.
    $SettingNotConfiguredAllowed = $false  # Set to true if STIG allows for setting to be Not Configured.
...
Function Get-V246877 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246877
        STIG ID    : HRZC-7X-000003
        Rule ID    : SV-246877r768591_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must not show the Log in as current user option.
    #>
    ...
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security"  # Registry path identified in STIG
    $RegistryValueName = "LogInAsCurrentUser_Display"  # Value name identified in STIG
    $RegistryValue = @("false")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Display option to Log in as current user"  # GPO setting name identified in STIG
    $SettingState = "Disabled"  # GPO configured state identified in STIG.
    $SettingNotConfiguredAllowed = $false  # Set to true if STIG allows for setting to be Not Configured.
...
Function Get-V246878 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246878
        STIG ID    : HRZC-7X-000004
        Rule ID    : SV-246878r768594_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must not ignore certificate revocation problems.
    #>
    ...
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security"  # Registry path identified in STIG
    $RegistryValueName = "IgnoreRevocation"  # Value name identified in STIG
    $RegistryValue = @("false")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Ignore certificate revocation problems"  # GPO setting name identified in STIG
    $SettingState = "Disabled"  # GPO configured state identified in STIG.
    $SettingNotConfiguredAllowed = $false  # Set to true if STIG allows for setting to be Not Configured.
...
Function Get-V246879 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246879
        STIG ID    : HRZC-7X-000005
        Rule ID    : SV-246879r768597_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must require TLS connections.
    #>
    ...
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security"  # Registry path identified in STIG
    $RegistryValueName = "EnableTicketSSLAuth"  # Value name identified in STIG
    $RegistryValue = @("3")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Enable SSL encrypted framework channel"  # GPO setting name identified in STIG
    $SettingState = "Enabled with Enforced"  # GPO configured state identified in STIG.
    $SettingNotConfiguredAllowed = $false  # Set to true if STIG allows for setting to be Not Configured.
...
Function Get-V246880 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246880
        STIG ID    : HRZC-7X-000006
        Rule ID    : SV-246880r768600_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must use approved ciphers.
    #>
    ...
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security"  # Registry path identified in STIG
    $RegistryValueName = "SSLCipherList"  # Value name identified in STIG
    $RegistryValue = @("TLSv1.2:!aNULL:kECDH+AESGCM:ECDH+AESGCM:RSA+AESGCM:kECDH+AES:ECDH+AES:RSA+AES")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Configures SSL protocols and cryptographic algorithms"  # GPO setting name identified in STIG
...
Function Get-V246881 {
    <#
    .DESCRIPTION
        Vuln ID    : V-246881
        STIG ID    : HRZC-7X-000007
        Rule ID    : SV-246881r768603_rule
        CCI ID     : CCI-000366
        Rule Name  : SRG-APP-000516-AS-000237
        Rule Title : The Horizon Client must not allow command line credentials.
    #>
    ...
    $TempUserHivePath = ""  # User's loaded hive to perform check
    $RegistryPath = "HKLM:\Software\Policies\VMware, Inc.\VMware VDM\Client\Security"  # Registry path identified in STIG
    $RegistryValueName = "AllowCmdLineCredentials"  # Value name identified in STIG
    $RegistryValue = @("false")  # Value expected in STIG (if REG_DWORD/REG_QWORD use hex and remove leading 0x000...)
    $RegistryType = "REG_SZ"  # Value type expected in STIG
    $SettingName = "Allow command line credentials"  # GPO setting name identified in STIG
    $SettingState = "Disabled"  # GPO configured state identified in STIG.
    $SettingNotConfiguredAllowed = $false  # Set to true if STIG allows for setting to be Not Configured.
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAYpFsvPZ9ofU3b
# 4IWed/loBfRvw2RRCMmidgSUHmQb5qCCCTswggR6MIIDYqADAgECAgQDAgTXMA0G
...
# BgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCDag4pCz0xAAa/v9qXDPaRIn/Ieuj2V
# Z7gY6YOUO9+NgzANBgkqhkiG9w0BAQEFAASCAQDPWGDPTa8na9aidNM24+QQI6nl
# F/BSkQ+bvrB3kHOTBVAXCpVd1T9Ox8DJCd7EpVMwi27V50OjwBYH+fXFHmzryMCe
# brhLM3MeHiHTutS0UDroDc/NZOnLLBBk4hSdz6mEfzZWNPZSZ1ztolFcLMcX2O85
# /dMS3bZ7KJ3nxC4j2LblwuaK7o7NV/tY257lao1VmHotsMcqdXswf9UxlIY1DR+E
# hylj4DrE6aRigS6GIiiT5G1ZpPkE0+zGiBImeBOgQOhC5lmMW68KwUhzgDiHboyR
# +hc7OINLxwImcTWbiIVNnps0ntLFP4KK8ojt8jsTAOCpxtvvFIOAEqik6Fpj
