...
#function Move-NessusFiles () {
 <#
  ...
  #The following modules will be ran 
    ...
    $AllFolders = @("$destDirA","$destDirB","$destdirC","$destDirD","$destDirE","$destDirF")
...
# Checking if directories exist if not creating them
    ...
    elseif ($chkthisMonth -eq $true){
        Remove-Item -Path $thisMonthPath -Recurse
        write-host "Removing superseded updates"
        }
    else {
    ...
    Get-ChildItem $nsocDir -Filter *.zip | Expand-Archive -DestinationPath $AllFolders -Force
    Set-Location -Path $nsocDir 
    Remove-Item '.\*.zip' -Force
        {
...
#}#End of Function
# Move files to directory older than X days
