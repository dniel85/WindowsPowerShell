function Get-ODAuthentication
{
	...
	.PARAMETER Scope
	Comma seperated string defining the authentication scope (https://dev.onedrive.com/auth/msa_oauth.htm). Default: "onedrive.readwrite,offline_access"
	.PARAMETER RefreshToken
	Refreshs the authentication token unattended with this refresh token. 
	.PARAMETER AutoAccept
	In token mode the accept button in the web form is pressed automatically
	.PARAMETER RedirectURI
	Don't use this parameter for the token authentication. Default is https://login.live.com/oauth20_desktop.srf. Code authentication requieres a correct url. Use the same as in the app registration. E.g. http://localhost/logon
	...
	} else
	{
		...
		if ($Logout)
		{
			$URIGetAccessToken="https://login.microsoftonline.com/logout.srf"
		}
		else
		{
		...
		$DocComplete  = {
			if ($web.Url.AbsoluteUri -match "access_token=|error|code=") {$form.Close() }
