...
Function Download-AVdefs{
    Param(
        ...
        [validateScript({
        if (-not (Test-Path $_ )) {
        throw [system.io.filenotfoundexception] "$_ is not a valid file path."
        }
        return $true   
        })]
        [string]$Path,
    ...
    function Download-File {
        ...
        Write-Log -level INFO -message "[$fun] starting function download-file with param's $url and $destinationfolder" -LogEnabled:$LogEnabled
        try{
            $fileName = [System.IO.Path]::Combine($destinationFolder, [System.IO.Path]::GetFileName($url)) 
            Write-Log -level INFO -message "[$fun] combine destination folder and url using variable $filename" -LogEnabled:$LogEnabled
            Invoke-WebRequest -Uri $url -OutFile $fileName 
            Write-Log -level INFO -message "[$fun] Invoking webrequest and downloading file to correct filename" -LogEnabled:$LogEnabled
        } 
        catch{
            Write-Log -level ERROR -message "[$fun] Error downloading" $url : $_ -LogEnabled:$LogEnabled
        }
    }
    ...
    function Get-DownloadableLinks {
        ...
        try{
            ...
            Write-Log -level ERROR -message "[$fun] Error retrieving links from" $url : $_
            return @()
        }
    }
    ...
    foreach ($url in $urls){
    Write-Log -level INFO -message "[$fun]  Putting Urls into foreach loop" -LogEnabled:$LogEnabled
    ...
    Write-Log -level INFO -message "[$fun] Parsing Foldernames to be created for child directories" -LogEnabled:$LogEnabled
    ...
    if (-not (test-path -Path $downloadDir\$folderName)) {
        Write-Log -level WARN -message "[$fun] $downloadDir does not exist!! creating child directory:: $folderName" -LogEnabled:$LogEnabled
        New-Item -ItemType Directory -Path $downloadDir\$folderName | Out-Null
        }
    
    # puts all downloadable links into a variable 
    $downloadableLinks = Get-DownloadableLinks -url $url
    Write-Log -level INFO -message "[$fun] Calling Function Get-downloadable links and placing int variable `$downloadableLinks" -LogEnabled:$LogEnabled
    ...
    Write-Log -level INFO -message "[$fun] Calling function Download-file" -LogEnabled:$LogEnabled
